...
MAP2_TRUE = False
MAP3_TRUE = False
CHARACTER1_MENU = True
CHARACTER2_MENU = False
HOME_MENU = False
DEAD_MENU = False
environment = []
background = 0
...
def character1_menu():
    global HOME_MENU
    global CHARACTER1_MENU
    global CHARACTER2_MENU
    global camera
    global DEAD_MENU
    global player1_images

    option1_images = uvage.load_sprite_sheet("pixel_character_dark_blue.png", 8, 8)
    option2_images = uvage.load_sprite_sheet("pixel_character_dark_green.png", 8, 8)
    option3_images = uvage.load_sprite_sheet("pixel_character_dark_red.png", 8, 8)
    option4_images = uvage.load_sprite_sheet("pixel_character_dark_yellow.png", 8, 8)
    option5_images = uvage.load_sprite_sheet("pixel_character_pale_blue_original.png", 8, 8)
    option6_images = uvage.load_sprite_sheet("pixel_character_pale_green.png", 8, 8)
    option7_images = uvage.load_sprite_sheet("pixel_character_pale_red.png", 8, 8)
    option8_images = uvage.load_sprite_sheet("pixel_character_pale_yellow.png", 8, 8)

    if CHARACTER1_MENU:
        screen = uvage.from_text(400, 100, "Welcome to GunFight!", 100, 'red')
        character_choose = uvage.from_text(400, 150, 'Click the number under a character for player 1!', 40, 'blue')
        player1_option1 = uvage.from_image(50, 300, option1_images[0])
        player1_option1_indicator = uvage.from_text(50, 340, "a", 50, "red")
        player1_option2 = uvage.from_image(150, 300, option2_images[0])
        player1_option2_indicator = uvage.from_text(150, 340, "b", 50, "red")
        player1_option3 = uvage.from_image(250, 300, option3_images[0])
        player1_option3_indicator = uvage.from_text(250, 340, "c", 50, "red")
        player1_option4 = uvage.from_image(350, 300, option4_images[0])
        player1_option4_indicator = uvage.from_text(350, 340, "d", 50, "red")
        player1_option5 = uvage.from_image(450, 300, option5_images[0])
        player1_option5_indicator = uvage.from_text(450, 340, "e", 50, "red")
        player1_option6 = uvage.from_image(550, 300, option6_images[0])
        player1_option6_indicator = uvage.from_text(550, 340, "f", 50, "red")
        player1_option7 = uvage.from_image(650, 300, option7_images[0])
        player1_option7_indicator = uvage.from_text(650, 340, "g", 50, "red")
        player1_option8 = uvage.from_image(750, 300, option8_images[0])
        player1_option8_indicator = uvage.from_text(750, 340, "h", 50, "red")

        if uvage.is_pressing('a'):
            player1_images = option1_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('b'):
            player1_images = option2_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('c'):
            player1_images = option3_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('d'):
            player1_images = option4_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('e'):
            player1_images = option5_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('f'):
            player1_images = option6_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('g'):
            player1_images = option7_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True
        if uvage.is_pressing('h'):
            player1_images = option8_images
            CHARACTER1_MENU = False
            CHARACTER2_MENU = True

        camera.draw(player1_option1)
        camera.draw(player1_option1_indicator)
        camera.draw(player1_option2)
        camera.draw(player1_option2_indicator)
        camera.draw(player1_option3)
        camera.draw(player1_option3_indicator)
        camera.draw(player1_option4)
        camera.draw(player1_option4_indicator)
        camera.draw(player1_option5)
        camera.draw(player1_option5_indicator)
        camera.draw(player1_option6)
        camera.draw(player1_option6_indicator)
        camera.draw(player1_option7)
        camera.draw(player1_option7_indicator)
        camera.draw(player1_option8)
        camera.draw(player1_option8_indicator)
        camera.draw(screen)
        camera.draw(character_choose)
...
def character2_menu():
    ...
    if CHARACTER2_MENU:
        screen = uvage.from_text(400, 100, "Welcome to GunFight!", 100, 'red')
        character_choose = uvage.from_text(400, 150, 'Click the number under a character for player 2!', 40, 'blue')
        player2_option1 = uvage.from_image(50, 300, option1_images[0])
        player2_option1_indicator = uvage.from_text(50, 340, "i", 50, "blue")
        player2_option2 = uvage.from_image(150, 300, option2_images[0])
        player2_option2_indicator = uvage.from_text(150, 340, "j", 50, "blue")
        player2_option3 = uvage.from_image(250, 300, option3_images[0])
        player2_option3_indicator = uvage.from_text(250, 340, "k", 50, "blue")
        player2_option4 = uvage.from_image(350, 300, option4_images[0])
        player2_option4_indicator = uvage.from_text(350, 340, "l", 50, "blue")
        player2_option5 = uvage.from_image(450, 300, option5_images[0])
        player2_option5_indicator = uvage.from_text(450, 340, "m", 50, "blue")
        player2_option6 = uvage.from_image(550, 300, option6_images[0])
        player2_option6_indicator = uvage.from_text(550, 340, "n", 50, "blue")
        player2_option7 = uvage.from_image(650, 300, option7_images[0])
        player2_option7_indicator = uvage.from_text(650, 340, "o", 50, "blue")
        player2_option8 = uvage.from_image(750, 300, option8_images[0])
        player2_option8_indicator = uvage.from_text(750, 340, "p", 50, "blue")

        if uvage.is_pressing('i'):
            player2_images = option1_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('j'):
            player2_images = option2_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('k'):
            player2_images = option3_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('l'):
            player2_images = option4_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('m'):
            player2_images = option5_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('n'):
            player2_images = option6_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('o'):
            player2_images = option7_images
            CHARACTER2_MENU = False
            HOME_MENU = True
        if uvage.is_pressing('p'):
            player2_images = option8_images
            CHARACTER2_MENU = False
            HOME_MENU = True

        camera.draw(player2_option1)
        camera.draw(player2_option1_indicator)
        camera.draw(player2_option2)
        camera.draw(player2_option2_indicator)
        camera.draw(player2_option3)
        camera.draw(player2_option3_indicator)
        camera.draw(player2_option4)
        camera.draw(player2_option4_indicator)
        camera.draw(player2_option5)
        camera.draw(player2_option5_indicator)
        camera.draw(player2_option6)
        camera.draw(player2_option6_indicator)
        camera.draw(player2_option7)
        camera.draw(player2_option7_indicator)
        camera.draw(player2_option8)
        camera.draw(player2_option8_indicator)
        camera.draw(screen)
        camera.draw(character_choose)
...
def move_player1():
    ...
    if player1_alive == True:
        ...
        if uvage.is_pressing("space"):
            ...
            player1_shootingframe += 0.3
            if player1_shootingframe >= 44:
                player1_shootingframe = 40
            player1.image = player1_images[int(player1_shootingframe)]
...
def move_player2():
    ...
    if player2_alive == True:
        ...
        if uvage.is_pressing("m"):
            ...
            player2_shootingframe += 0.3
            if player2_shootingframe >= 36:
                player2_shootingframe = 32
            player2.image = player2_images[int(player2_shootingframe)]
